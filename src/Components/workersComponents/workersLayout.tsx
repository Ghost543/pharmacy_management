...
export const WorkersLayout: FC<{ data: string }> = ({ data }) => {
    ...
    useEffect(() => {
        ...
        if (workerTab === "online") {
            setTabWorkers(
                workers.filter(w => w.status === WorkerStatus.online)
            );
        }
        if (workerTab === "offline") {
            setTabWorkers(
                workers.filter(w => w.status === WorkerStatus.offline)
            );
        }
    }, [workerTab]);
    ...
    return (
        <div
            className={`w-full min-h-screen flex flex-col gap-2 ${inconsolata.className} relative`}>
            {/*{ data }*/}
            <div
                className={`w-full max-h-fit flex flex-row p-4 gap-2 justify-between items-center`}>
                <div
                    className={`w-1/2 max-h-fit flex flex-row justify-start items-center`}>
                ...
                <div
                    className={`w-1/2 max-h-fit flex flex-row justify-end items-center`}>
                    <span
                        onClick={() => openModal()}
                        ...
                        <MdPersonAdd className={`text-lg text-gray-400`} /> Add
                        Worker
                    </span>
                    {modal && (
                        <div
                            ...
                            <div
                                ...
                                <div
                                    ...
                                    <div
                                        className={`w-4/5 max-h-fit flex flex-col gap-2.5 p-2`}>
                                        <input
            ...
            <div
                className={`w-full max-h-fit flex shadow flex-row justify-end items-center p-4 bg-gray-800/40  gap-2`}>
                <select
                    name="status"
                    className={`bg-transparent ring-1 ring-gray-500 w-36 h-6 placeholder:font-medium placeholder-gray-600 placeholder:text-sm text-xs text-gray-300  p-2 outline-0 border-none focus:border-none focus:ring-gray-300 focus:outline-none`}>
                ...
                />
                <div
                    className={`w-fit  max-h-fit flex flex-row items-center gap-2 ring-1 ring-gray-500 pr-2`}>
                    <input
                        type={"text"}
                        name={`search`}
            ...
            <div
                className={`w-full max-h-fit bg-gray-800/40 flex flex-col gap-2 p-2`}>
                <ul
                    className={`w-full max-h-fit flex flex-row justify-start gap-2 text-gray-400 text-base shadow`}>
                    <li
                        onClick={() => setWorkerTab("all")}
                        className={`w-fit h-fit p-2  pb-0 cursor-pointer ${
                            workerTab === "all"
                                ? "text-blue-500 shadow-blue-600/70 shadow-sm"
                                : ""
                        }`}>
                        All
                    </li>
                    <li
                        ...
                        className={`w-fit max-h-fit flex flex-row gap-2 justify-center items-center p-2 pb-0 cursor-pointer ${
                            workerTab === "online"
                                ? "text-blue-500 shadow-blue-600/70 shadow-sm"
                                : ""
                        }`}>
                        <span className={`relative flex h-2 w-2`}>
                            <span
                                className={`animate-ping absolute inline-flex h-full w-full bg-sky-400 opacity-75 rounded-full`}></span>
                            <span
                                className={`relative inline-flex rounded-full h-2 w-2 bg-sky-500`}></span>
                        </span>{" "}
                        Online
                    </li>
                    <li
                        onClick={() => setWorkerTab("offline")}
                        className={`w-fit max-h-fit flex flex-row gap-1 justify-center items-center p-2 pb-0 cursor-pointer ${
                            workerTab === "offline"
                                ? "text-blue-500 shadow-blue-600/70 shadow-sm"
                                : ""
                        }`}>
                        <span className={`relative flex h-2 w-2`}>
                            <span
                                className={`animate-ping absolute inline-flex h-full w-full bg-rose-400 opacity-75 rounded-full`}></span>
                            <span
                                className={`relative inline-flex rounded-full h-2 w-2 bg-rose-500`}></span>
                        </span>{" "}
                        Offline
                    </li>
                </ul>
